...
async function loadLanguageColors() {
  ...
  try {
    ...
    for (const [name, config] of Object.entries(languages)) {
      if (config.color) languageColors[name] = config.color
    }
    cachedLanguageColors = languageColors
    fs.writeFileSync(CACHE_FILE, JSON.stringify({ timestamp: Date.now(), colors: languageColors }, null, 2))
    console.log(`Loaded ${Object.keys(languageColors).length} language colors`)
    ...
    try {
      ...
      if (!response.ok) {
        if (attempt < maxAttempts) {
          ...
          console.log(`  Attempt ${attempt}/${maxAttempts} failed (${response.status}), retrying in ${Math.ceil(delayMs / 1000)}s...`)
      ...
      if (data.errors) {
        if (isTransientGraphQLError(data.errors) && attempt < maxAttempts) {
          const delayMs = Math.min(1000 * Math.pow(2, attempt - 1), 20000)
          console.log(`  Attempt ${attempt}/${maxAttempts} failed (transient error), retrying in ${Math.ceil(delayMs / 1000)}s...`)
          await new Promise(resolve => setTimeout(resolve, delayMs))
          continue
        }
      ...
      if (attempt < maxAttempts) {
        const delayMs = Math.min(1000 * Math.pow(2, attempt - 1), 20000)
        console.log(`  Attempt ${attempt}/${maxAttempts} failed, retrying in ${Math.ceil(delayMs / 1000)}s...`)
        await new Promise(resolve => setTimeout(resolve, delayMs))
        continue
      }
      throw err
  ...
  while (hasNextPage) {
    ...
    try {
      ...
      if (/GraphQL HTTP 50[234]/.test(String(err.message || ''))) {
        console.log('  Gateway error while fetching repos. Using partial list.')
        break
      }
      throw err
    }
    ...
    for (const repo of repoNodes) {
      ...
      if (commitCount > 0) {
        ...
        repos.push({ name: repo.name, url: repo.url, commits: commitCount, languages, additions: 0, deletions: 0 })
      }
    }
...
async function fetchRepoCommitStats(token, owner, repoName, userId, since) {
  ...
  while (hasNextPage) {
    ...
    try {
      ...
      if (/GraphQL HTTP 50[234]/.test(String(err.message || ''))) {
        console.log(`    Gateway error for ${repoName}. Skipping additions/deletions.`)
        break
      }
      throw err
    }
  ...
  for (const repo of repos) {
    for (const lang of repo.languages) {
      if (!languageCommits[lang.name]) {
        languageCommits[lang.name] = { name: lang.name, weightedCommits: 0, color: lang.color }
      }
...
function generateAdditionsBadge(formattedNum) {
  const encoded = encodeURIComponent(`+${formattedNum}`)
  return `![+${formattedNum}](https://img.shields.io/static/v1?style=flat-square&label=&color=3fb950&message=${encoded})`
}

function generateDeletionsBadge(formattedNum) {
  const encoded = encodeURIComponent(`-${formattedNum}`)
  return `![-${formattedNum}](https://img.shields.io/static/v1?style=flat-square&label=&color=f85149&message=${encoded})`
}
...
function processTemplate(template, data) {
  ...
  result = result.replace(/{{\s*TOTAL_ADDITIONS_LAST_YEAR\s*}}/g, generateAdditionsBadge(formatNumber(data.totalAdditionsLastYear)))
  result = result.replace(/{{\s*TOTAL_DELETIONS_LAST_YEAR\s*}}/g, generateDeletionsBadge(formatNumber(data.totalDeletionsLastYear)))
  ...
  if (repoTemplateMatch) {
    ...
    const repoItems = data.topRepos.map(repo => {
      ...
      item = item.replace(/{{\s*REPO_ADDITIONS\s*}}/g, generateAdditionsBadge(formatNumber(repo.additions)))
      item = item.replace(/{{\s*REPO_DELETIONS\s*}}/g, generateDeletionsBadge(formatNumber(repo.deletions)))
      return item.trimEnd()
    }).join('\n')
...
async function main() {
  ...
  const accountAge = Math.floor((new Date() - accountCreatedAt) / (365.25 * 24 * 60 * 60 * 1000))
  ...
  const yearsToFetch = cacheFresh ? years.filter(year => year > cacheYear) : years
  ...
  if (!cacheFresh) {
    const cachePayload = {
      ...
      yearly: Object.fromEntries(Object.entries(mergedYearly).filter(([year]) => Number(year) <= cacheYear))
    }
  ...
  const statsData = {
    ...
    topLanguagesRows: (() => {
      ...
      const lastYearRows = [
        ...
        `${generateAdditionsBadge(formatNumber(totalAdditionsLastYear))} lines added`,
        `${generateDeletionsBadge(formatNumber(totalDeletionsLastYear))} lines removed`
      ]
      for (let i = 0; i < 5; i++) {
  ...
  try {
    ...
    console.error('TEMPLATE.md not found!')
    process.exit(1)
  }
